...
export function AdminSettingsEditor() {
  ...
  return (
    <div className="mt-6 grid gap-3">
      {rows.length === 0 ? (
        <article className="rounded-sm border border-[#b78d4b2d] bg-white p-4 text-[#6f6251]">No settings saved yet.</article>
      ) : (
        rows.map((setting) => (
          <article key={setting.id} className="rounded-sm border border-[#b78d4b2d] bg-white p-4">
            ...
            <textarea
              ...
              className="mt-2 min-h-[140px] w-full rounded-sm border border-[#b78d4b35] bg-[#fffaf4] p-3 font-mono text-xs text-[#4f4335]"
            />
          </article>
        ))
      )}
      <div className="flex items-center gap-3">
        <button onClick={save} className="rounded-sm bg-[#b78d4b] px-5 py-2 text-sm text-white">
          Save All Settings
        </button>
