  ...
  useEffect(() => {
    void fetch("/api/partner/earnings")
      .then(async (res) => {
        ...
        setLedger(payload.ledger.filter((e) => e.sourceType === "SERVICE"));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className={adminTitle}>Visit earnings</h1>
        <p className={adminMuted}>
          Commission for completed service visits. Entries become eligible when you mark a booking complete; admin
          includes them in payouts.
        </p>
      </div>
      {error ? <p className="text-sm text-[#7c2c2c]">{error}</p> : null}
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {[
          ["Visit gross", summary?.gross],
          ["Total commission", summary?.commission],
          ["Eligible for payout", summary?.eligible],
          ["Pending completion", summary?.pending],
        ].map(([label, value]) => (
          ...
          <tbody>
            {ledger.map((entry) => (
              <tr key={entry.id} className="border-b border-[#f0e8db]">
                ...
                <td className="px-4 py-3 text-[#1f1a15]">{entry.description ?? "Visit"}</td>
            ...
            {!ledger.length ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-[#6f6251]">
                  No visit earnings yet. Complete assigned bookings to unlock payout eligibility.
                </td>
              </tr>
            ) : null}
          </tbody>
