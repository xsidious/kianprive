...
import { editorialCtaPrimary, editorialInput, editorialPanel } from "@/components/ui/editorial-primitives";
...
export function BlogGrid({ posts }: { posts: BlogPost[] }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
          ...
          {categories.map((category) => (
            <button
              ...
              className={`rounded-sm border px-3 py-2 text-left text-sm ${
                selectedCategory === category ? "border-[#b78d4b] bg-[#fff6e8] text-[#8f6f3e]" : "border-[#e4d9c8] bg-white text-[#4f4335]"
              }`}
            >
              {category}
            </button>
      ...
      <div>
        <div className={`mb-6 grid gap-3 ${editorialPanel} p-4 md:grid-cols-[1fr_auto]`}>
          <input
            value={query}
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((post) => (
            <article key={post.slug} className={`overflow-hidden ${editorialPanel}`}>
              ...
              <div className="p-6">
                ...
                <h2 className="mt-3 font-serif text-2xl text-[#2b2218]">{post.title}</h2>
                <p className="mt-3 text-[#6f6251]">{post.excerpt}</p>
                <Link href={`/blog/${post.slug}`} className={`mt-5 ${editorialCtaPrimary}`}>
                  READ ARTICLE
                </Link>
              </div>
            </article>
