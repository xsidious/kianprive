...
type Props = {
  ...
  placeholder?: string;
  submitLabel?: string;
  loadMessages: () => Promise<OrderThreadMessage[]>;
  sendMessage: (body: string) => Promise<OrderThreadMessage>;
  initialMessages?: OrderThreadMessage[];
  reloadKey?: string | number;
  pollIntervalMs?: number;
  /** Highlight "your" side of the thread */
  selfRole: "CUSTOMER" | "ADMIN";
};

function normalizeList(rows: unknown): OrderThreadMessage[] {
  if (!Array.isArray(rows)) return [];
  return rows.filter((row): row is OrderThreadMessage => {
    return Boolean(row && typeof row === "object" && "id" in row && "body" in row);
  });
}

function mergeById(prev: OrderThreadMessage[], next: OrderThreadMessage[]) {
  if (!next.length && prev.length) return prev;
  const map = new Map<string, OrderThreadMessage>();
...
export function OrderMessageThread({
  ...
  hint = "Ask about shipping, tracking, or your order. Our team replies here.",
  ...
  const [messages, setMessages] = useState<OrderThreadMessage[]>(() => normalizeList(initialMessages));
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-[#e7dcc8] bg-[#fcfaf6] p-5">
      ...
      >
        ...
        {!loading && messages.length === 0 ? (
          <p className="text-sm text-[#8a7d6c]">No messages yet on this order.</p>
        ) : null}
        {messages.map((msg) => {
          const fromSelf = msg.authorRole === selfRole;
          return (
            <article
              key={msg.id}
              className={`rounded-lg px-3 py-2 text-sm ${
                fromSelf ? "ml-6 bg-[#f3f0ea] text-[#1f1a15]" : "mr-6 bg-[#f7f1e6] text-[#1f1a15]"
              }`}
            >
